...
const Feature5SvgComponent = () => {
  ...
  // Child variants for subtle bounce effect in central elements
  const childVariants = {
    hidden: { opacity: 0, scale: 0.95, y: 20 },
    visible: {
      opacity: 1,
      scale: [1, 1.02, 1], // Gentle bounce
      y: 0, // Reset y to 0 for centering, no constant offset
      x: 0, // Reset x to 0 for proper alignment
      transition: {
        duration: 1.5,
        ease: "easeOut",
        scale: {
  ...
  // Panel compression/spread animation (for corner shapes) - refined for smoother coordination and fixed bug with over-movement
  const panelVariants = {
    hidden: { opacity: 0, scale: 1, x: 0, y: 0 },
    visible: {
      opacity: 1,
      scale: [1, 0.9, 1], // Adjusted compression for more natural feel without distortion
      x: [0, 40, 0], // Reduced movement to prevent clipping or overlap issues
      transition: {
        duration: 2.8, // Slightly longer for smoother sync with ripples
        repeat: Infinity,
        repeatType: "reverse",
        ease: "easeInOut",
        delay: 0.3, // Adjusted delay for better wave coordination
      },
    },
  };
  ...
  const rightPanelVariants = {
    ...
    visible: {
      opacity: 1,
      scale: [1, 0.9, 1], // Matching left panel
      x: [0, -40, 0], // Reduced movement to prevent clipping or overlap issues
      transition: {
        duration: 2.8, // Slightly longer for smoother sync with ripples
        repeat: Infinity,
  ...
  // Circle ripple animation (outward wave from box) - refined for more fluid wave-like effect and added a third ripple for depth
  const circleRippleVariants = {
    hidden: { scale: 0.6, opacity: 0 }, // Smoother start
    visible: {
      scale: [0.6, 1.6, 2.2], // Adjusted for more controlled expansion
      opacity: [0.7, 0.3, 0], // Softer fade for realistic wave
      transition: {
        duration: 2.8, // Synced with panels
        repeat: Infinity,
        repeatType: "loop",
        ease: "easeOut",
        delay: 0.6, // Staggered for outer wave
      },
    },
  };

  const innerCircleRippleVariants = {
    ...
    visible: {
      ...
      transition: {
        ...
        repeatType: "loop",
        ease: "easeOut",
        delay: 0.2, // Inner starts earlier
      },
    },
  };

  const outerCircleRippleVariants = {
    ...
    visible: {
      ...
      transition: {
        ...
        repeatType: "loop",
        ease: "easeOut",
        delay: 0.9, // Outermost delayed for layered waves
      },
    },
  };

  // Dashed line chain-like animation - refined duration for sync and fixed potential offset bug
  const dashedLineVariants = {
    ...
    visible: {
      ...
      transition: {
        strokeDashoffset: {
          duration: 2.8, // Synced with others
          repeat: Infinity,
          repeatType: "loop",
          ease: "linear",
        },
    ...
    >
      <g clipPath="url(#clip0_490_713)">
        ...
        <motion.g
          ...
          <motion.circle 
            // @ts-ignore
            variants={outerCircleRippleVariants}
            opacity="0.1"
            cx="245"
            cy="164.303"
            r="155.633" // Added outer ripple for more depth
            stroke="white"
            strokeOpacity="0.25"
            strokeWidth="2"
          />
          ...
          <motion.circle
            // @ts-ignore 
            variants={innerCircleRippleVariants}
            opacity="0.15"
            cx="245"
            cy="164.303"
            r="116.555"
