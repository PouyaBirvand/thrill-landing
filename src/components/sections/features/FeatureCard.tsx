...
export default function FeatureCard({
  ...
  return (
    <div
      className={`relative w-full max-w-[490px] transition-all duration-700 ease-out ${isExpanded
    ...
    >
      ...
      {/* Content Container - NO card hover effects */}
      <div className={`relative w-full h-full rounded-[20px] ${isExpanded && 'rounded-[34px]'} bg-[#F8F8F805] flex flex-col overflow-hidden`}>
        ...
        <div className="flex-shrink-0 mt-2 relative">
          ...
          {isModal && onClose && (
            <button
              onClick={onClose}
              className="absolute top-3 right-5 flex-shrink-0 bg-white/10 hover:bg-[#83FFDA20] hover:border-[#83FFDA40] p-1.5 xs:p-2 sm:p-2 rounded-full border border-white/5 transition-all duration-200 hover:scale-110 hover:rotate-90"
            ...
            >
              <X
                ...
                color={isIconHovered ? "#83FFDA" : "#858FA660"}
                style={{
                  filter: isIconHovered ? "drop-shadow(0 0 8px rgba(131, 255, 218, 0.6))" : "none"
                }}
              />
            </button>
          )}
        ...
        <div className="flex-1 flex flex-col px-4 xs:px-5 sm:px-6 pb-6 xs:pb-4 sm:pb-7 sm:pt-0 pt-3">
          ...
          {!isModal && (
            <div className="flex items-center justify-between flex-shrink-0 pt-5">
              <h2 className={`text-neutral-white text-nowrap font-semibold uppercase leading-[1.15] flex-1 pr-3 xs:pr-4 ${isModal
              ...
              {onClose ? (
                ...
                >
                  <Plus
                    size={14}
                    className="xs:w-4 xs:h-4 sm:w-4 sm:h-4 transition-all duration-300"
                    color={isExpanded ? "#83FFDA" : (isIconHovered ? "#83FFDA" : "#858FA660")}
                    style={{
                      filter: isIconHovered ? "drop-shadow(0 0 8px rgba(131, 255, 218, 0.6))" : "none"
                    }}
                  />
          ...
          <div className={`flex-1 overflow-hidden transition-all duration-700 ease-out ${isExpanded
            ...
            <div className={`h-full flex flex-col justify-between ${isModal
              ...
              <div className="flex-1 mb-3 xs:mb-4 sm:mb-5 md:mb-6">
                <p className={`text-neutral-lightGray leading-relaxed ${isModal
                  ? "text-[10px] xs:text-xs sm:text-sm md:text-base"
                  : "text-xs xs:text-sm sm:text-base md:text-lg"
                  }`}>
                  {description}
                </p>
                <p className={`text-neutral-lightGray mt-2 ${isModal
                  ? "text-[10px] xs:text-xs sm:text-sm md:text-base"
                  : "text-xs xs:text-sm sm:text-base md:text-lg"
                  }`}>
                  {more_detail}
              ...
              <div className="grid grid-cols-2 gap-2 xs:gap-3 sm:gap-4 md:gap-5 flex-shrink-0">
                <div className="flex flex-col space-y-0.5 xs:space-y-1 sm:space-y-1.5">
                  <span className={`text-white/70 font-bold leading-tight ${isModal
                  ...
                  <span className={`text-neutral-lightGray leading-relaxed ${isModal
